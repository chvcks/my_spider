...
class Station:
    ...
    def __init__(self, fromStation, toStation, date, favoriteTrain):
        ...
        self.favorite_train = favoriteTrain
    ...
    def query_ticket(self):
        ...
        if resp.status_code == 200 and resp.url == full_url:
            ...
            self._get_train_info(resp.json(), from_station, to_station, self.favorite_train)

        else:
            print('query_ticket() error! status_code:{}, url:{}\norigin_url:{}'
                  .format(resp.status_code, resp.url, full_url))

    def _get_train_info(self, text, from_station, to_station, favorite_train):
        """
        功能:提取出查询到的列车信息
        ...
        if result.get('data').get('map'):
            ...
            for item in train_info:
                ...
                #if split_item[11] == 'Y' and split_item[3] == favorite_train: # 已经开始卖票了
                if split_item[3] == favorite_train:
    ...
    def _print_train(self, train_info, from_station, to_station):
        ...
        print('{}, {}从{}到{}:'.format(self.train_date, self.favorite_train,from_station, to_station))
        for item in train_info:
    ...
    def _query_prompt(self):
        ...
        with open(filename, 'r') as f:
            ...
            stations = data.keys()
            if self.from_station not in stations or self.to_station not in stations:
                station_flag = False
                print('query_prompt() error: {}或{}不在站点列表中!!'
                    .format(self.from_station, self.to_station))
            # 获取起始站和终点站的代码
            from_station_code = data.get(self.from_station)
            to_station_code = data.get(self.to_station)
        query_data['train_date'] = self.train_date
        query_data[self.from_station] = from_station_code
        query_data[self.to_station] = to_station_code

        if station_flag:
            return query_data
        else:
            print('query_prompt() error! station_flag:{}'
                  .format(station_flag))
...
def main():
    queryList = {("上海", "新沂", "2018-08-02", "K8356"),
                 ("新沂", "上海", "2018-08-03", "K8358"),
                 ("天津", "上海", "2018-08-06", "T109"),
                 ("天津", "上海", "2018-08-03", "Z281"),
                 ("上海", "天津", "2018-08-03", "Z172"),
                 ("上海", "天津", "2018-08-05", "Z282")}
    for queryItem in queryList:
        Station(queryItem[0], queryItem[1], queryItem[2], queryItem[3]).query_ticket()
        print("="*100)
